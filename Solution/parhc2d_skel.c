...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *xleftghost, double *xrightghost, double *sendbuf_x, double *recvbuf_x)
{
  ...
  // set left neighbours 
  if (rank_x == 0)
    left_nb = MPI_PROC_NULL;  // no left neighbor for leftmost processors
  else
    left_nb = rank - 1;

  // set right neighbours 
  if (rank_x == px-1)
    right_nb = MPI_PROC_NULL;  // no right neighbor for rightmost processors
  else
    right_nb = rank + 1;
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  // set bot neighbours 
  if (rank_y == 0)
    bot_nb = MPI_PROC_NULL;  // no bottom neighbor for bottommost processors
  else
    bot_nb = rank - px;

  // set top neighbours 
  if (rank_y == py-1)
    top_nb = MPI_PROC_NULL;  // no top neighbor for topmost processors
  else
    top_nb = rank + px;
...
void linsolve_hc2d_gs_adi(int nx, int ny, double rx, double ry, double **rhs, double **T, double **Tnew)
{
    ...
    for (j = 1; j < ny - 1; j++)
    {
        ...
        for (i = 1; i < nx - 1; i++)
        {
            ...
            b[i] = 1.0 + 2.0 * rx;
            c[i] = -rx;
            d[i] = T[i][j] + ry * (T[i][j + 1] - 2.0 * T[i][j] + T[i][j - 1]);
        }
    ...
    for (i = 1; i < nx - 1; i++)
    {
        ...
        double *c = (double *)malloc(ny * sizeof(double));
        double *d = (double *)malloc(ny * sizeof(double));
        ...
        for (j = 1; j < ny - 1; j++)
        {
            ...
            b[j] = 1.0 + 2.0 * ry;
            c[j] = -ry;
            d[j] = Tnew[i][j] + rx * (Tnew[i + 1][j] - 2.0 * Tnew[i][j] + Tnew[i - 1][j]);
        }
...
int main(int argc, char** argv)
{
  ...
  // printf("Total time for %d steps: %f seconds\n", num_time_steps, total_time);
  // printf("Average time per time step: %f seconds\n", total_time / num_time_steps);
